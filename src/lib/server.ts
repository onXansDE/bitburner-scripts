...
export class ServerHandler {
	...
	public canNuke(server: Server): boolean {
		...
		if (server.purchasedByPlayer) {
			this.ns.print(
				`INFO Server ${server.hostname} cant be nuked because it is owned by the player`
			);
			return false;
		}
		...
		if (
			...
			server.hackDifficulty > currentHackingLevel
		) {
			this.ns.print(
				`WARN Server ${server.hostname} cant be hacked because the hacking level is too low`
			);
			return false;
		} else if (
			server.numOpenPortsRequired === undefined ||
			currentHackingToolCount < server.numOpenPortsRequired
		) {
			this.ns.print(
				`WARN Server ${server.hostname} cant be hacked because the hacking tool count is too low`
			);
			return false;
		}
