...
export async function main(ns) {
	...
	if (server.purchasedByPlayer) {
		ns.tprint(
			`Server ${servername} cant be tracked because it is owned by the player`
		);
		return;
	}
	ns.tail();
	...
	while (true) {
		...
		ns.print(
			`Server: ${servername} (${server.ip}, ${server.organizationName})`
		);
		ns.print(
			`Security: ${tf.getFormatedInfo(
				server,
				DisplayPreset.DIFFICULTY_PERCENT
			)}`
		);
		ns.print(
			`Money: ${tf.getFormatedInfo(
				server,
				DisplayPreset.MONEY_FULL_PERCENT
			)} ${tf.getTrendFormated(moneyHistory)}`
		);
		ns.print(``);
